 ...
 *
 * Skeleton sizing: when we already have at least one ranked entry
 * (e.g. fallback data from the RSC pre-fetch is being revalidated)
 * the skeleton matches that count so the list doesn't visibly
 * reshuffle when the new data arrives. Otherwise we render the full
 * `MAX_ROSTER_ROWS` rows. Both branches are pure functions of the
 * props, so server and client first render agree — no hydration trap.
 */
